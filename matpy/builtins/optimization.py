...
def _quadprog(H, f, A=None, b=None, Aeq=None, beq=None, lb=None, ub=None):
    ...
    if A is not None and b is not None:
        ...
        constraints.append(
            {"type": "ineq", "fun": lambda x: b_data.flatten() - A_data @ x}
        )
    if Aeq is not None and beq is not None:
        ...
        constraints.append(
            {"type": "eq", "fun": lambda x: beq_data.flatten() - Aeq_data @ x}
        )
    bounds = None
...
def _intlinprog(f, intcon, A=None, b=None, Aeq=None, beq=None, lb=None, ub=None):
    ...
    if Aeq is not None and beq is not None:
        ...
        constraints.append(
            LinearConstraint(Aeq_data, beq_data.flatten(), beq_data.flatten())
        )
    bounds = None
    if lb is not None or ub is not None:
...
def _fminbnd(func, a, b, *args):
    ...
    result = scipy_optimize.minimize_scalar(
        func, bounds=(a, b), method="bounded", args=args
    )
...
def _linprog(c, A=None, b=None, Aeq=None, beq=None, lb=None, ub=None):
    ...
    A_eq = (
        Aeq.data if isinstance(Aeq, Mat) else np.array(Aeq) if Aeq is not None else None
    )
    b_eq = (
        beq.data if isinstance(beq, Mat) else np.array(beq) if beq is not None else None
    )
    bounds = None
    if lb is not None or ub is not None:
        lb_val = float(lb) if lb is not None else None
...
def _fminimax(func, x0, *args):
    ...
    x0 = x0.flatten()
    result = scipy_optimize.minimize(
        lambda x: np.max(np.abs(func(x, *args))), x0, method="Nelder-Mead"
    )
    return Mat(result.x), float(result.fun)
