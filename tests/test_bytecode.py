...
class TestControlFlow:
    ...
    def test_while_loop(self):
        result, vm = run_bytecode("i = 0; s = 0; while i < 10; i = i + 1; s = s + i; end")
        assert vm.variables.get('s') == 55
...
class TestUserFunctions:
    def test_simple_function(self):
        ...
        assert vm.variables.get('result') == 25 or (hasattr(vm.variables.get('result'), 'data') and vm.variables.get('result').data.flat[0] == 25)
    ...
    def test_function_with_multiple_ops(self):
        ...
        """
        result, vm = run_bytecode(source)
        assert vm.variables.get('result') == 20 or (hasattr(vm.variables.get('result'), 'data') and vm.variables.get('result').data.flat[0] == 20)
